...
JOIN items_ingredients ii ON ii.item_id = it.id 
JOIN ingredients i ON i.ingredient_id = ii.ingredient_id
WHERE i.supplier_name = 'Anna Food Distributors'

SELECT f.id, SUM(i.price * ig.quantity) + (f.preparation_time + f.serving_time) * 0.2 AS cost
FROM foods f
