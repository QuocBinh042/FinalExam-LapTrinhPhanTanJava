select b.ISBN from books b join reviews r on r.ISBN=b.ISBN 
join books_authors ba on ba.ISBN=r.ISBN 
where ba.author='Robert C. Martin' and r.rating>=0

UPDATE reviews
SET rating = ?, comment = ?
WHERE ISBN = ? AND person_id = ?
AND rating BETWEEN 1 AND 5
AND comment IS NOT NULL AND comment != '';

SELECT ba.author, COUNT(*) AS book_count FROM book_translations bt JOIN books_authors ba ON bt.ISBN = ba.ISBN GROUP BY ba.author ORDER BY ba.author
